...
function Sider({cambioVentana, ventana}){
  ...
  const cerrarSesion = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/logout`, {
        method: 'POST',
        credentials: 'include',
      });
      if (!response.ok) {
  ...
  return(
    <div className='sider-rumbo'>
      <div className='sider-perfil-usuario' onClick={()=>cambioVentana('perfil')}>
        <div className='sider-perfil-avatar'><AvatarPublicacion urlImagen={perfil.avatar_url}/></div><p>{perfil.full_name}</p>
      </div>
      <MenuOption 
      texto={'Inicio'} 
      ...
      <MenuOption 
      texto={'Recursos'} 
      icono={<IconoInformacion color={ventana=='recursos'?'white':''}/>} 
      ...
      funcion={()=> window.open("https://www.ipn.mx/oferta-educativa/educacion-superior/")} 
      ...
      funcion={()=> window.open("https://comparacarreras.imco.org.mx/?gad_campaignid=1463017670")}
      />
