...
import { useObtencionUsuario, useObtencionUsuarioExterno } from '../../hooks/obtencionUsuario.js';
...
function VentanaPerfil(){
  ...
  useEffect(() => {
    if (imagenSeleccionada) {
      if (editando === 'banner') {
        setObjetosEditables({ ...objetosEditables, banner_url: imagenSeleccionada });
      } else if (editando === 'avatar') {
        setObjetosEditables({ ...objetosEditables, avatar_url: imagenSeleccionada });
      }
    }
  }, [imagenSeleccionada]);
  ...
  const handleBioChange = (e) => {
    setError(false);
    setMensaje('');
    setObjetosEditables({ ...objetosEditables, bio: e.target.value });
  };

  const handleUsernameChange = (e) => {
    setError(false);
    setMensaje('');
    setObjetosEditables({ ...objetosEditables, username: e.target.value });
  };
  ...
  const handleEditar = () => {
    ...
    try {
      fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/actualizar-perfil`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <div className="contenedor-ventana-perfil">
      <div className='contenedor-perfil-banner'>
        ...
        >
          {editar && miPerfil && (
            <div className="banner-editar-overlay" onClick={() => seleccionarImagen('banner')}>✎</div>
          )}
        </div>

        <div className={`contenedor-imagen-ventana-perfil ${editar ? 'editar' : ''}`}>
          <AvatarPublicacion urlImagen={previewAvatar || perfil.avatar_url} className={editar ? 'avatar-imagen-perfil' : ''} />
          {editar && miPerfil && (
            <div className="avatar-editar-overlay" onClick={() => seleccionarImagen('avatar')}>✎</div>
          )}
        </div>
        ...
        <div className='contenedor-datos-perfil'>
          <div className='datos-perfil'>
            <h1>
              {editar ? 
                <InputInvisible
                  ...
                  onChange={(e) => setObjetosEditables({...objetosEditables, full_name: e.target.value})}
                  maxLength={20}
                />
                : perfil.full_name
              }
            ...
            <div className='contenedor-botones-perfil'>
              { miPerfil && (!editar ? 
                <IconoEditar funcion={()=>{setEditar(true)}}/> 
                : <>
                    <ButtonOutline text='Cancelar' funcion={handleCancelar} className='boton-etiquetas' enable={true} />
                    <Button text={subiendoImagen ? 'Subiendo...' : 'Guardar'} action={handleEditar} className='boton-editar-perfil' enable={!subiendoImagen && !error} />
                  </>
              )}
            </div>
          </div>
          ...
          <div className='contenedor-descripcion-perfil'>
            <p>{editar
              ? <InputInvisible holder={perfil.bio} maxLength={200} className='input-bio-perfil' value={objetosEditables.bio} onChange={(e) => setObjetosEditables({...objetosEditables, bio: e.target.value})} />
              : perfil.bio
            }</p>
          </div>
        </div>
