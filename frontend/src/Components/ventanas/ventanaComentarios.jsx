...
function VentanaComentarios({idPublicacion, showComentarios, setShowComentarios}){
  ...
  const obtenerComentarios = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/post-comments?post_id=${idPublicacion}`, {
        method: 'GET',
        headers: {
  ...
  const handleContinuar = async () => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/post-comment`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  return(
    <div className='contenedor-ventana-comentarios'>
      <div className='contenedor-comentarios'>
        {comentarios.length === 0 ? <p style={{marginTop: '1rem', color: '#1f1f1f'}}>Se el primero en comentar</p> :  [...comentarios].reverse().map((comentario) => (<CardComentario key={comentario.id} objeto={comentario}/>))}
      </div>
      <div className='contenedor-crear-comentario'>
        ...
        value={comentario}
        id={'comentario'}
        /> <div className='contenedor-icono-imagen'><IconoEnviar funcion={handleContinuar}/></div> 
      </div>
    </div>
  )
}
