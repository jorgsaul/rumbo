...
export default function CascadeMenu({
  ...
  const obtenerEtiquetas = async () => {
    try {
      const resultado = await fetch(`${import.meta.env.VITE_APP_API_BASE_URL}/etiquetas`);
        ...
        <div className="cascade-menu">
          {etiquetas.map((categoria) => (
            <div key={categoria.category_id}>
              <p style={{color: categoria.category_color, fontWeight: 'bold'}}>
              ...
              {categoria.etiquetas.map((etiqueta) => (
                <Checkbox 
                  ...
                  checked={selectedEtiquetas[categoria.category_id]?.includes(etiqueta.id) || false}
                  accion={() => handleCheckboxChange(categoria.category_id, etiqueta.id)}
                />
              ))}
            </div>
          ))}
